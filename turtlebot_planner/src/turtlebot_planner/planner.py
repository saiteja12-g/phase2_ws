...
def get_route(path, r, L):
    route = []
    for cell in path:
        movement = cell.previous_cell_movement
        if movement is None:
            continue
        message = Twist()
        message.linear.x = r*(movement[0]+movement[1])/120.0 
        message.angular.z = r*(movement[1]-movement[0])/(60.0*L)
...
class Matrix:
    ...
    def build(self, start_cell):
        ...
        while len(current_cells) != 0 and not rospy.is_shutdown():
            new_cells = []
